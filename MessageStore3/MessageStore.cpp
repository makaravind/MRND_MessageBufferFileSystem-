...
void ReadReplies(struct message message, int msg_id);
int GetMessageId(int message_offset);
void PrintMessage(int offset);
void New_Reply(struct category category, int msg_id, char *reply);
void InserNewReply(int message_offset, char *reply, int msg_id);
int New_Reply_Create(char *reply, int uid, int msg_id);
struct message GetMessage(struct category category, int msg_id);
//
void InserNewReply(int message_offset, char *reply, int msg_id){
	...
	for (int i = 0; i < INDIRECT1_MESSAGES; i++){
		if (indirect.messages[i] == 0){
			indirect.messages[i] = reply_created_at;
			break;
		}
	}
	// fseeek back and update the indirect
	fseek(fp, message.indirect, SEEK_SET);
	fwrite(&indirect, sizeof(indirect1), 1, fp);
	
	//fseek to the offset and update the message object
	fseek(fp, message_offset, SEEK_SET);
...
void New_Reply(struct category category, int msg_id, char *reply){
	...
	for (int i = 0; i < 3; i++){
		...
		if (message_id == msg_id){
			...
			InserNewReply(category.messages[i], reply, msg_id);
			break;
		}
	}
		...
		for (int i = 0; i < INDIRECT1_MESSAGES; i++){
			if (indirect.messages[i] > 0){
				...
				if(message_id == msg_id){
					flag = 1;
					// enter the reply here
					InserNewReply(indirect.messages[i], reply, msg_id);
					break;
				}
			}
		}
...
struct message GetMessage(struct category category, int msg_id){
	...
	struct indirect1 indirect;
	for (int i = 0; i < 3; i++){
		if (category.messages[i] > 0){
			fseek(fp, category.messages[i], SEEK_SET);
			fread(&message, sizeof(struct message), 1, fp);
			if (message.id == msg_id) return message;
		}
	}
	// fseek to indiret
	fseek(fp, category.indirect, SEEK_SET);
	fread(&indirect, sizeof(struct indirect1), 1, fp);
	for (int i = 0; i < INDIRECT1_MESSAGES; i++){
		if (indirect.messages[i] > 0){
			fseek(fp, indirect.messages[i], SEEK_SET);
			fread(&message, sizeof(struct message), 1, fp);
			if (message.id == msg_id) return message;
		}
	}
...
void ReadReplies(struct category category, int msg_id){
	...
	struct indirect1 indirect;
	struct reply reply;
	message = GetMessage(category,msg_id);
	// fseek to indirect
	fseek(fp, message.indirect, SEEK_SET);
	fread(&indirect, sizeof(struct indirect1), 1, fp);
	for (int i = 0; i < INDIRECT1_MESSAGES; i++){
		if (indirect.messages[i] > 0){
			fseek(fp, indirect.messages[i], SEEK_SET);
			fread(&reply, sizeof(struct reply), 1, fp);
			if (reply.id == msg_id)
				printf("%s\n", reply.text);
		}
	}
...
void ReadMessages(struct category category){
	...
	case 1:
		...
		char reply[128];
		printf("enter your reply here:");
		scanf("%s", &reply);
		New_Reply(category, msg_selection, reply);
		break;
	case 2:
		ReadReplies(category, msg_selection);
		break;
	default:
		Error("choice");
		break;
...
int New_Reply_Create(char *reply, int uid, int msg_id){
	...
	*/
	main_block_message_creation_offset = BLOCK_SIZE * 0 + (BLOCK_SIZE * 3);
	...
	strcpy(new_reply.text, reply);
	new_reply.uid = uid;
	new_reply.id = msg_id;

	//write the actual replu
	fseek(fp, in_block_message_creation_offset, SEEK_SET);
	fwrite(&new_reply, sizeof(struct message), 1, fp);
